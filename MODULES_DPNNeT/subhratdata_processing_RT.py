...
def unseen_data_test_alt(path, image_name, classifier, regressor_1, regressor_2, regressor_3, X_res, Y_res):
...
#     print(img)
    ...
    a = np.argmax(classifier.predict(img_data), axis = 1)
    if a == 0:
        print("one planet system", regressor_1.predict(img_data))
    elif a == 1:
        print("two planet system", regressor_2.predict(img_data))
    else:
        print("three planet system", regressor_3.predict(img_data))